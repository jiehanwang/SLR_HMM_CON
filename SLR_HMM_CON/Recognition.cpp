...
void CRecognition::ContinueTestOneSen_Whj(double **feature, int nFrames, char* result)
{//连续识别一个句子
//输入文件名
//输入文件是否归一化, TRUE 为 需要归一，
//输出结果
	...
	//head = ContinuostRecog(FileName,&nFrames, bNeedCalibrate);
	head = ContinuostRecog_Whj(feature,nFrames);
...
Linklists *CRecognition::ContinuostRecog_Whj(double **feature,int frames)
{
	//double **data;
	//FILE *fp;
	int nFrames, nDimension;
...
////////////////////////////////////////////////////////////
	...
	for(t=0; t<nFrames; t++)  
	{//第一次的head 
		PreSlectCandidate(head,feature[t],IndexU,&ivt,NewFirstProb,t,
						ActiveWordList, &ActiveWordNum, 
						StateSize);//IndexU store previous value, ivt previous num

	    head=CreateLists(head, IndexU, ivt, ActiveWordList, ActiveWordNum, 
			           StateSize,MaxState, t, feature[t], NewFirstProb,
						m_pDhmm->m_nTotalHmmWord, nDimension);

		//int indexword = head->pWordList[1];
// 		cout<<ActiveWordNum<<"---";
// 		for (int i=0; i<ActiveWordNum; i++)
// 		{
// 			cout<<ActiveWordList[i]<<" ";
// 		}
// 		cout<<endl;
		//cout<<m_pDhmm->m_pHmmWordIndex[ActiveWordList[0]]->Word<<" ";
...
bool CRecognition::continuous_initial(void)
{
	nFramesNow = 0;
...
void CRecognition::continuous_recog(double *feature, int frameID)
{
	PreSlectCandidate(head,feature,IndexU,&ivt,NewFirstProb,frameID,
		ActiveWordList, &ActiveWordNum, 
		StateSize);//IndexU store previous value, ivt previous num
...
void CRecognition::continuous_release(void)
{
	delete []StateSize;
	delete []IndexU;
	delete []NewFirstProb;
