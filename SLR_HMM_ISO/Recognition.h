...
public:
	...
	void ContinueTestOneSen_Whj(double **feature, int nFrames, char* result);
	//连续识别一个句子
...
public:
	...
	Linklists *ContinuostRecog_Whj(double **feature,int frames);
...
public:
	...
	double * NewFirstProb;
	bool continuous_initial(void);
	void continuous_recog(double *feature, int frameID);
	void continuous_release(void);
	void continuous_loop(double *feature, int frameID, char* result);
};
